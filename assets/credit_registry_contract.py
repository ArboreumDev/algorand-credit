...
def credit_program():
    """
    This stateful smart contract writes credit profiles to users local state
    """
    ...
    # Code block invoked during credit-profile creation(TODO update).
    # Only the registrar
    # may invoke this block with two arguments and one account supplied.
    # The first argument was "new_profile" used by the control flow 
    # below. The second argument is the credit profile metadata which is
    # set to the local storage of the supplied account (Int(1))
    diploma_metadata = Txn.application_args[1]
    write_profile = Seq([
        Assert(is_registrar),
        Assert(Txn.application_args.length() == Int(2)),
        App.localPut(Int(1), Bytes("credit"), diploma_metadata),
        Return(Int(1))
    ])
    ...
    #     App.localDel(Int(1), Bytes("credit")),
    #     Return(Int(1))
    # ])
...
if __name__ == "__main__":
    print(compileTeal(credit_program(), Mode.Application, version=3))